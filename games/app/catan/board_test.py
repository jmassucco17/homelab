...
from games.app.catan.models import board, player, serializers
...
class TestResources(unittest.TestCase):
    ...
    def test_total(self) -> None:
        res = player.Resources(wood=2, brick=1, wheat=0, sheep=3, ore=1)
    ...
    def test_get(self) -> None:
        res = player.Resources(ore=5)
        self.assertEqual(res.get(board.ResourceType.ORE), 5)
        self.assertEqual(res.get(board.ResourceType.WOOD), 0)

    def test_can_afford_settlement(self) -> None:
        res = player.Resources(wood=1, brick=1, wheat=1, sheep=1)
        self.assertTrue(res.can_afford(player.SETTLEMENT_COST))

    def test_can_afford_dev_card(self) -> None:
        res = player.Resources(wheat=1, sheep=1, ore=1)
        self.assertTrue(res.can_afford(player.DEV_CARD_COST))
        res_no_ore = player.Resources(wheat=1, sheep=1)
        self.assertFalse(res_no_ore.can_afford(player.DEV_CARD_COST))
...
class TestDevCardHand(unittest.TestCase):
    ...
    def test_total(self) -> None:
        hand = player.DevCardHand(knight=3, victory_point=1)
    ...
    def test_add(self) -> None:
        ...
        hand2 = hand.add(player.DevCardType.KNIGHT)
    ...
    def test_remove(self) -> None:
        ...
        hand2 = hand.remove(player.DevCardType.ROAD_BUILDING)
...
class TestBoardGenerator(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.board = board_generator.generate_board(seed=42)
    ...
    def test_tile_type_distribution(self) -> None:
        ...
        counts: dict[board.TileType, int] = {}
        for tile in self.board.tiles:
            counts[tile.tile_type] = counts.get(tile.tile_type, 0) + 1
        self.assertEqual(counts[board.TileType.FOREST], 4)
        self.assertEqual(counts[board.TileType.PASTURE], 4)
        self.assertEqual(counts[board.TileType.FIELDS], 4)
        self.assertEqual(counts[board.TileType.HILLS], 3)
        self.assertEqual(counts[board.TileType.MOUNTAINS], 3)
        self.assertEqual(counts[board.TileType.DESERT], 1)
    ...
    def test_desert_has_no_number(self) -> None:
        ...
        desert = next(
            t for t in self.board.tiles if t.tile_type == board.TileType.DESERT
        )
    ...
    def test_robber_starts_on_desert(self) -> None:
        ...
        self.assertEqual(robber_tile.tile_type, board.TileType.DESERT)
    ...
    def test_port_type_distribution(self) -> None:
        ...
        counts: dict[board.PortType, int] = {}
        for port in self.board.ports:
            counts[port.port_type] = counts.get(port.port_type, 0) + 1
        self.assertEqual(counts.get(board.PortType.GENERIC, 0), 4)
        specific_ports = {
            board.PortType.WOOD,
    ...
    def test_deterministic_with_seed(self) -> None:
        ...
        b1 = board_generator.generate_board(seed=7)
        b2 = board_generator.generate_board(seed=7)
        self.assertEqual(
    ...
    def test_balanced_board(self) -> None:
        ...
        board = board_generator.generate_board(balanced=True, seed=99)
...
class TestBoardSerialization(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.board = board_generator.generate_board(seed=1)

    def test_serialize_model(self) -> None:
        """serialize_model must return a plain dict."""
        data = serializers.serialize_model(self.board)
...
class TestPlayerModel(unittest.TestCase):
    ...
    def _make_player(self) -> player.Player:
    ...
    def test_player_serialization(self) -> None:
        player = self._make_player()
        data = serializers.serialize_model(player)
