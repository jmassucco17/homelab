...
class TestApp(unittest.TestCase):
    """Tests for games FastAPI application."""
    ...
    def test_index_page_contains_game_links(self) -> None:
        """Test index page links to all games."""
        response = self.client.get('/')
        self.assertEqual(response.status_code, 200)
        self.assertIn('/snake', response.text)
        self.assertIn('/pong', response.text)
        self.assertIn('/catan', response.text)
