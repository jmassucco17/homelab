 ...
 * @param {number} q
 * @param {number} r
 * @param {number} size  hex radius in logical px
...
const PIP_COUNTS = { 2: 1, 3: 2, 4: 3, 5: 4, 6: 5, 8: 5, 9: 4, 10: 3, 11: 2, 12: 1 }
...
export class CatanBoardRenderer {
  ...
  constructor(canvas) {
    ...
    /** @type {object|null} serialised Board from the server */
    this.board = null
    ...
    /** @type {Record<number, {x:number,y:number}>} */
    this.edgePixels = {}
    /** @type {Array<{x:number,y:number}>} indexed by tile_index */
    this.tileCenters = []
    ...
    /** @type {string[]} CSS colour per player_index */
    this.playerColors = ['#e63946', '#457b9d', '#2a9d8f', '#e9c46a']

    // Interaction callbacks
    /** @type {((vertexId: number) => void) | null} */
    this.onVertexClick = null
    /** @type {((edgeId: number) => void) | null} */
    this.onEdgeClick = null
    /** @type {((tileIndex: number) => void) | null} */
  ...
  _computeGeometry() {
    const positions = this.board.tiles.map((t) => [t.coord.q, t.coord.r, t.coord.s])
  ...
  _onTouchMove(e) {
    e.preventDefault()
    if (e.touches.length === 1 && this._lastPanPos) {
  ...
  _lighten(hex, amount) {
    ...
    const l = (c) => Math.min(255, c + Math.round((255 - c) * amount))
