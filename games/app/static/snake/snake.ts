...
let snake: Array<{ x: number; y: number }> = [] // [{x, y}, …] — head is index 0
let dir: { x: number; y: number } = { x: 1, y: 0 } // current direction
let nextDir: { x: number; y: number } = { x: 1, y: 0 } // queued direction (applied at next tick)
...
// ---------------------------------------------------------------------------
const BTN_DIRS: Record<string, { x: number; y: number }> = {
  'btn-up': { x: 0, y: -1 },
  'btn-down': { x: 0, y: 1 },
  'btn-left': { x: -1, y: 0 },
  'btn-right': { x: 1, y: 0 },
